...
class MainWindow(CTk):
    def __init__(self):
        ...
        # Налаштування сервера (змінюйте тут для віддаленого підключення)
        self.server_host = "localhost"  # Змініть на URL з Railway, наприклад "your-app.railway.app"
        self.server_port = 8080  # Зазвичай Railway використовує 8080, але перевірте
        ...
        try:
            ...
            self.add_message(f"Не вдалося підключитися до сервера {self.server_host}:{self.server_port}: {e}")

    def on_closing(self):
        if hasattr(self, 'sock') and self.sock:
            try:
                leave_msg = f"TEXT@{self.username}@[LEAVE] Покинув чат.\n"
    ...
    def open_image(self):
        ...
        try:
            ...
            self.sock.sendall(data.encode())
            self.add_message(f"{self.username} надіслав(ла) зображення: {short_name}", CTkImage(light_image=Image.open(file_name), size=(300, 300)))
        except Exception as e:
